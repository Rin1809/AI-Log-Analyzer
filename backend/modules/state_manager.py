...
def get_last_run_timestamp(host_id, test_mode=False):
    ...
    state_file = f".{prefix}last_run_timestamp_{host_id}"
...
def get_last_cycle_run_timestamp(host_id, test_mode=False):
    prefix = "test_" if test_mode else ""
    state_file = f".{prefix}last_cycle_run_{host_id}"
    if os.path.exists(state_file):
        with open(state_file, 'r') as f:
            try:
                return datetime.fromisoformat(f.read().strip())
...
def get_stage_buffer_count(host_id, stage_index, test_mode=False):
    ...
    prefix = "test_" if test_mode else ""
    count_file = f".{prefix}buffer_count_{host_id}_{stage_index}"
...
def reset_all_states(host_id, test_mode=True):
    ...
    prefix = "test_"
    files_to_delete = [
        f".{prefix}last_run_timestamp_{host_id}",
        f".{prefix}last_cycle_run_{host_id}",
    ]
    
    for f in os.listdir('.'):
        if f.startswith(f".{prefix}buffer_count_{host_id}"):
